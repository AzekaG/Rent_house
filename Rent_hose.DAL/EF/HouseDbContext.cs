        ...
        public HouseDbContext(DbContextOptions<HouseDbContext> options) : base(options) 
        {
            if (Database.EnsureCreated())
            {
                ...
                Houses?.AddRange(
                    new House
                    {
                        District = Districts?.Where(x => x.Name == "Black District").FirstOrDefault(),
                        Name = "Обьявление номер 1",
                        Area = 333,
                        CountFloor = 3,
                        CountRoom = 12,
                        Description = "Super magic House",
                        Subscribe = 3,
                        Price = 10000000,
                        Is_Valid = true,
                ...
                Users?.AddRange(
                    new User
                    {
                        Status = Statuses.Where(x => x.Name == "admin").FirstOrDefault(),
